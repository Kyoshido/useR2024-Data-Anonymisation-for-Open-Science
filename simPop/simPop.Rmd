...
# Packages
```{r laod packages, warning=FALSE, message=FALSE}
library(simPop)
...
calibrated to known marginals, the resulting population will not exactly match these marginals
...
#   data = combined_data,
#   method = "class",
#   control = rpart.control(minsplit = 5, cp = 0.001, maxdepth = 30)
# )
...
combined_data <- combined_data |> mutate(
  propensity_score_log_reg = predict(res_logreg, type = "response"),
  # propensity_score_CART    = predict(res_cart, type = "prob")[,2]
  # propensity_score_rf = predict(res_rf, type = "response")
  )


ggplot(combined_data, aes(x = propensity_score_log_reg, color = as.factor(group), fill = as.factor(group))) + 
  geom_density(alpha = 0.5) +
  labs(color = "Group", fill = "Group") +
  theme_minimal()

pmse <- mean((combined_data$propensity_score_log_reg - combined_data$group)^2)

```


# Confidentiality (inference attack)
Scenario Background Knowledge: The intruder has complete information on region, household size, age, gender, economic status and citizenship
