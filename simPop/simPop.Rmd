...
```{r univariate mosaic,  fig.cap= Mosaic plots of sex, eco_stat}
tab <- spTable(simPop, select = c("sex", "eco_stat"))
spMosaic(tab, method = "color")
```

```{r CDF pgrossincome,  fig.cap= CDF pgross income}
spCdfplot(simPop, "pgrossincome", cond = "pgrossincome", layout = c(1, 2))
```
...
```{r}
true_pgrossincome$pgrossIncome - predict(inf_attack_model, newdata = intruder_knowledge)
```