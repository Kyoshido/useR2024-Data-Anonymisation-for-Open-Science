...
tab_2 <- calc_table(dat,dat_swapped_2,
                       keyVars=c("NUTS3","SEX","COC.L"))
tab_3 <- calc_table(dat,dat_swapped_3,
                       keyVars=c("NUTS3","SEX","COC.L"))

# mean absolute deviation per NUTS3
ad_2 <- tab_2[,mean(abs(N1-N2)),by=.(NUTS3)]
ad_2
ad_2[,mean(V1)] # mean over AD

# sum of RAD by NUTS3
rad_2 <- tab_2[,sum(abs(N1-N2)/N1),by=.(NUTS3)]
# take average
rad_2[,mean(V1)]

# hellingers distance by NUTS3
hd_2 <- tab_2[,sqrt(1/2*sum((sqrt(N1)-sqrt(N2))^2)),
               by=.(NUTS3)]
hd_2[,mean(V1)]

# mean absolute deviation per NUTS3
ad_3 <- tab_3[,mean(abs(N1-N2)),by=.(NUTS3)]
ad_3
ad_3[,mean(V1)] # mean over AD

# sum of RAD by NUTS3
rad_3 <- tab_3[,sum(abs(N1-N2)/N1),by=.(NUTS3)]
# take average
rad_3[,mean(V1)]

# hellingers distance by NUTS3
hd_3 <- tab_3[,sqrt(1/2*sum((sqrt(N1)-sqrt(N2))^2)),
               by=.(NUTS3)]
hd_3[,mean(V1)]

# 10% swaprate has higher information loss than 2.5% swaprate
...
cbind(ad_1,ad_2,ad_3)
cbind(ad_1[,mean(V1)],ad_2[,mean(V1)],ad_3[,mean(V1)])
cbind(rad_1,rad_2,rad_3)
cbind(rad_1[,mean(V1)],rad_2[,mean(V1)],rad_3[,mean(V1)])
cbind(hd_1,hd_2,hd_3)
cbind(hd_1[,mean(V1)],hd_2[,mean(V1)],hd_3[,mean(V1)])
